...
def supplies_value_at_date_for_keys(
    ...
    Zwraca mapę: (stock_type_id, name) -> remaining_value_at_date

    remaining_qty_at_date = supply.quantity - settled_qty(up to date) - sold_qty(up to date)
    remaining_value_at_date = supply.value * remaining_qty / supply.quantity
    """
    if not keys:
        return {}
    ...
    supplies = list(
        ...
        .only("id", "stock_type_id", "name", "quantity", "value", "date")
    )
...
class MonthlyWarehouseReportView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        received_supplies = (
            ...
            .filter(
                stock_type__stock_type="material",  # <- jeśli u Ciebie inaczej, zmień
                date__gte=start_date,
                date__lte=end_date,
            )
        )

        received_material_value = received_supplies.aggregate(
        ...
        for ss in received_supplies:
            ...
            received_material_m2 += Decimal(ss.quantity or 0) * dims_to_m2(dim)
        ...
        used_settlements = (
            ...
            .filter(
                as_result=False,
                stock_supply__stock_type__stock_type="material",  # <- jeśli u Ciebie inaczej, zmień
        ...
        processed_material_value = used_settlements.aggregate(
            s=Coalesce(Sum("value"), Decimal("0.00"))
        )["s"]
        ...
        for row in used_settlements:
            ...
            processed_material_m2 += Decimal(row.quantity or 0) * dims_to_m2(dim)
        processed_material_m2 = processed_material_m2.quantize(Decimal("0.001"), rounding=ROUND_HALF_UP)

        # --- sprzedaż (przychód + COGS + zysk) ---
        ...
        open_date_point = start_date - datetime.timedelta(days=1)  # stan otwarcia: koniec dnia poprzedniego
        value_start_map = supplies_value_at_date_for_keys(keys, open_date_point)
        value_end_map = supplies_value_at_date_for_keys(keys, end_date)
